...
[MitoGeneExtractor can be installed with conda/mamba](https://anaconda.org/channels/bioconda/packages/mitogeneextractor/overview). This automatically installs the required Exonerate program (2.4.0) as well:

```
conda install -c bioconda -c conda-forge mitogeneextractor
```

This provides the command `MitoGeneExtractor` (as well as the versioned name `MitoGeneExtractor-v1.9.6`).
...
### If installed via Bioconda:
Type
```
MitoGeneExtractor --help
```
...
> **Note: The "-vx.y.z" versioning used below is not needed if MitoGeneExtractor is installed via Bioconda**
