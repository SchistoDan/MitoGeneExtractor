...
Once the recipe has been accepted into [Bioconda](https://bioconda.github.io/), MitoGeneExtractor can be installed with conda/mamba. This automatically installs the required Exonerate program (2.4.0) as well:

```
conda install -c bioconda -c conda-forge mitogeneextractor
```

This provides the command `MitoGeneExtractor` (as well as the versioned name `MitoGeneExtractor-v1.9.6`). The Bioconda recipe is staged in the [`recipe/`](recipe/) directory of this repository.
